...
if (!empty($pro_config['order_day']) and $pro_config['order_nexttime'] < NV_CURRENTTIME) {
    $result = $db->query('SELECT order_id, order_time FROM ' . TABLE_SHOP_MAIN . '_orders WHERE transaction_status=0 AND order_time < ' . (NV_CURRENTTIME - (int) $pro_config['order_day'] * 86400));
    while (list ($order_id, $order_time) = $result->fetch(3)) {
        $result_del = $db->query('DELETE FROM ' . TABLE_SHOP_MAIN . '_orders WHERE order_id=' . $order_id);
        if ($result_del) {
...
function nv_set_status_module()
{
    ...
    $result = $db->query('SELECT id FROM ' . TABLE_SHOP_MAIN . '_rows WHERE status =2 AND publtime < ' . NV_CURRENTTIME . ' ORDER BY publtime ASC');
    while (list ($id) = $result->fetch(3)) {
        $db->query('UPDATE ' . TABLE_SHOP_MAIN . '_rows SET status =1 WHERE id=' . $id);
    }

    // Ngung hieu luc cac san pham da het han
    $result = $db->query('SELECT id, archive FROM ' . TABLE_SHOP_MAIN . '_rows WHERE status =1 AND exptime > 0 AND exptime <= ' . NV_CURRENTTIME . ' ORDER BY exptime ASC');
    ...
    $time_publtime = $db->query('SELECT MIN(publtime) FROM ' . TABLE_SHOP_MAIN . '_rows WHERE status =2 AND publtime > ' . NV_CURRENTTIME)->fetchColumn();

    $time_exptime = $db->query('SELECT MIN(exptime) FROM ' . TABLE_SHOP_MAIN . '_rows WHERE status =1 AND exptime > ' . NV_CURRENTTIME)->fetchColumn();
...
function nv_del_content_module($id)
{
    ...
    list ($id, $listcatid, $title) = $db->query('SELECT id, listcatid, ' . NV_LANG_DATA . '_title FROM ' . TABLE_SHOP_MAIN . '_rows WHERE id=' . intval($id))->fetch(3);
    if ($id > 0) {
        $number_no_del = 0;
        $array_catid = explode(',', $listcatid);
        if ($number_no_del == 0) {
            $sql = 'DELETE FROM ' . TABLE_SHOP_MAIN . '_rows WHERE id=' . $id;
            if (!$db->exec($sql)) {
                ++$number_no_del;
            }
        }
        if ($number_no_del == 0) {
            ...
            // Xoa block san pham
            $db->query('DELETE FROM ' . TABLE_SHOP_MAIN . '_block WHERE id = ' . $id);

            // Xoa du lieu nhom san pham
            $groupid = GetGroupID($id);
            if ($db->query('DELETE FROM ' . TABLE_SHOP_MAIN . '_group_items WHERE pro_id = ' . $id)) {
                // Xoa chi tiet nhap kho
                $db->query('DELETE FROM ' . TABLE_SHOP_MAIN . '_group_quantity WHERE pro_id = ' . $id);

                nv_fix_group_count($groupid);
            }

            // Xoa tai lieu
            $db->query('DELETE FROM ' . TABLE_SHOP_MAIN . '_files_rows WHERE id_rows=' . $id);

            // Xoa du lieu tuy bien
            $db->query('DELETE FROM ' . TABLE_SHOP_MAIN . '_field_value_' . NV_LANG_DATA . ' WHERE rows_id = ' . $id);
...
function nv_del_group($groupid)
{
    ...
    if ($db->query("DELETE FROM " . $db_config['prefix'] . "_" . $module_data . "_group WHERE groupid=" . $groupid)) {
        ...
        $db->query('DELETE FROM ' . TABLE_SHOP_MAIN . '_group_items WHERE group_id = ' . $groupid);

        // Xoa cateid
        $db->query('DELETE FROM ' . TABLE_SHOP_MAIN . '_group_cateid WHERE groupid = ' . $groupid);

        // Xoa chi tiet nhap kho, neu nhu chi tiet nhap kho co nhom nay, thi xoa luon chi tiet nhap kho
        $result = $db->query('SELECT * FROM ' . TABLE_SHOP_MAIN . '_group_quantity');
        while ($row = $result->fetch()) {
            if (in_array($groupid, explode(',', $listgroup))) {
                $db->query('DELETE FROM ' . TABLE_SHOP_MAIN . '_group_quantity WHERE pro_id = ' . $row['pro_id'] . ' AND listgroup=' . $db->quote($row['listgroup']));
            }
        }
...
{
    global $db, $module_data, $db_config;
    $db->query('UPDATE ' . TABLE_SHOP_MAIN . '_rows SET status =3 WHERE id=' . $id);
}
...
function product_number_order($listid, $listnum, $listgroup, $type = '-')
{
    ...
    foreach ($listid as $i => $id) {
        if ($id > 0) {
            ...
            $sql = 'UPDATE ' . TABLE_SHOP_MAIN . '_rows SET product_number = product_number ' . $type . ' ' . intval($listnum[$i]) . ' WHERE id =' . $id;
            $db->query($sql);

            if (!empty($listgroup)) {
                $sql = 'UPDATE ' . TABLE_SHOP_MAIN . '_group_quantity SET quantity = quantity ' . $type . ' ' . intval($listnum[$i]) . ' WHERE pro_id =' . $id . ' AND listgroup=' . $db->quote($listgroup[$i]);
                $db->query($sql);
            }
        }
    }
...
function product_number_sell($listid, $listnum, $type = '+')
{
    ...
    foreach ($listid as $i => $id) {
        if ($id > 0) {
            ...
            $sql = 'UPDATE ' . TABLE_SHOP_MAIN . '_rows SET num_sell = num_sell ' . $type . ' ' . intval($listnum[$i]) . ' WHERE id =' . $id;
            $db->query($sql);
        }
    }
}
...
function GetParentCatFilter($cateid)
{
    ...
    $cid = 0;
    while (true) {
        $count = $db->query('SELECT COUNT(*) FROM ' . TABLE_SHOP_MAIN . '_group_cateid WHERE cateid = ' . $cateid)->fetchColumn();
...
function GetGroupidInParentGroup($groupid, $check_inhome = 0, $only_children = 0, $cid = 0)
{
    ...
    if (!empty($subgroupid)) {
        foreach ($subgroupid as $id) {
            $result = $db->query('SELECT COUNT(*) FROM ' . TABLE_SHOP_MAIN . '_group_cateid WHERE groupid=' . $id . ' AND cateid =' . $cid)->fetch();
...
function GetGroupID($pro_id, $group_by_parent = 0)
{
    ...
    $data = array();
    $result = $db->query('SELECT group_id FROM ' . TABLE_SHOP_MAIN . '_group_items where pro_id=' . $pro_id);
    while ($row = $result->fetch()) {
        if ($group_by_parent) {
