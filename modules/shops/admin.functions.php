...
define('TABLE_SHOP_MAIN', $db_config['dbsystem'] . '.' . $db_config['prefix'] . '_' . $module_data);
...
function nv_fix_cat_order($parentid = 0, $order = 0, $lev = 0)
{
    ...
    $sql = 'SELECT catid, parentid FROM ' . TABLE_SHOP_MAIN . '_catalogs WHERE parentid=' . $parentid . ' and idsite = ' . IDSITE . ' ORDER BY weight ASC';
    $result = $db->query($sql);
    ...
    foreach ($array_cat_order as $catid_i) {
        ++$order;
        ++$weight;
        $sql = 'UPDATE ' . TABLE_SHOP_MAIN . '_catalogs SET weight=' . $weight . ', sort=' . $order . ', lev=' . $lev . ' WHERE catid=' . $catid_i . ' and idsite = ' . IDSITE;
        $db->query($sql);
    ...
    $numsubcat = $weight;
    if ($parentid > 0) {
        $sql = 'UPDATE ' . TABLE_SHOP_MAIN . '_catalogs SET numsubcat=' . $numsubcat;
        if ($numsubcat == 0) {
            $sql .= ", subcatid='', viewcat='viewcat_page_list'";
        } else {
            $sql .= ", subcatid='" . implode(",", $array_cat_order) . "'";
        }
        $sql .= ' WHERE catid=' . $parentid . ' and idsite = ' . IDSITE;
        $db->query($sql);
    }
    return $order;
}
...
function nv_fix_block_cat()
{
    ...
    $sql = 'SELECT bid FROM ' . TABLE_SHOP_MAIN . '_block_cat ORDER BY weight ASC';
    $weight = 0;
    $result = $db->query($sql);
    while ($row = $result->fetch()) {
        ++$weight;
        $sql = 'UPDATE ' . TABLE_SHOP_MAIN . '_block_cat SET weight=' . $weight . ' WHERE bid=' . $row['bid'];
        $db->query($sql);
    }
    $result->closeCursor();
}
...
function nv_news_fix_block($bid, $repairtable = true)
{
    ...
    if ($bid > 0) {
        $sql = 'SELECT id FROM ' . TABLE_SHOP_MAIN . '_block WHERE bid=' . $bid . ' ORDER BY weight ASC';
        $result = $db->query($sql);
        $weight = 0;
        while ($row = $result->fetch()) {
            ++$weight;
            if ($weight <= 500) {
                $sql = 'UPDATE ' . TABLE_SHOP_MAIN . '_block SET weight=' . $weight . ' WHERE bid=' . $bid . ' AND id=' . $row['id'];
            } else {
                $sql = 'DELETE FROM ' . TABLE_SHOP_MAIN . '_block WHERE bid=' . $bid . ' AND id=' . $row['id'];
            }
            $db->query($sql);
        }
        $result->closeCursor();
        ...
        if ($repairtable) {
            $db->query('REPAIR TABLE ' . TABLE_SHOP_MAIN . '_block');
        }
    }
}
...
function shops_show_cat_list($parentid = 0)
{
    ...
    if ($parentid > 0) {
        ...
        while ($parentid_i > 0) {
            list($catid_i, $parentid_i, $title_i) = $db->query('SELECT catid, parentid, ' . NV_LANG_DATA . '_title FROM ' . TABLE_SHOP_MAIN . '_catalogs WHERE catid=' . intval($parentid_i) . ' and idsite = ' . IDSITE)->fetch(3);
		...
		$sql = 'SELECT catid, parentid, ' . NV_LANG_DATA . '_title, weight, viewcat, numsubcat, inhome, numlinks, newday FROM ' . TABLE_SHOP_MAIN . '_catalogs WHERE parentid=' . $parentid . ' and idsite = ' . IDSITE . ' ORDER BY weight ASC';
    $result = $db->query($sql);
    $num = $result->rowCount();
    ...
    if ($num > 0) {
        ...
        while (list($catid, $parentid, $title, $weight, $viewcat, $numsubcat, $inhome, $numlinks, $newday) = $result->fetch(3)) {
            ...
            if (!array_key_exists($viewcat, $array_viewcat)) {
                ...
                $stmt = $db->prepare('UPDATE ' . TABLE_SHOP_MAIN . '_catalogs SET viewcat= :viewcat WHERE catid=' . $catid . ' and idsite = ' . IDSITE);
...
function nv_fix_group_order($parentid = 0, $sort = 0, $lev = 0)
{
    ...
    $sql = 'SELECT groupid, parentid FROM ' . TABLE_SHOP_MAIN . '_group WHERE parentid=' . $parentid . ' ORDER BY weight ASC';
    $result = $db->query($sql);
    ...
    foreach ($array_group_order as $groupid_i) {
        ...
        $sql = 'UPDATE ' . TABLE_SHOP_MAIN . '_group SET weight=' . $weight . ', sort=' . $sort . ', lev=' . $lev . ' WHERE groupid=' . $groupid_i;
        $db->query($sql);
...
function shops_show_location_list($parentid = 0, $page, $per_page, $base_url)
{
    ...
    // Fetch Limit
    $db->sqlreset()->select('COUNT(*)')->from(TABLE_SHOP_MAIN . '_location')->where('parentid = ' . $parentid);
...
function nv_fix_location_order($parentid = 0, $sort = 0, $lev = 0)
{
    ...
    $sql = 'SELECT id, parentid FROM ' . TABLE_SHOP_MAIN . '_location WHERE parentid=' . $parentid . ' ORDER BY weight ASC';
    $result = $db->query($sql);
    ...
    foreach ($array_location_order as $locationid_i) {
        ...
        $sql = 'UPDATE ' . TABLE_SHOP_MAIN . '_location SET weight=' . $weight . ', sort=' . $sort . ', lev=' . $lev . ' WHERE id=' . $locationid_i;
        $db->query($sql);
...
function nv_show_block_list($bid)
{
    ...
    $sql = 'SELECT t1.id, t1.listcatid, t1.' . NV_LANG_DATA . '_title, t1.' . NV_LANG_DATA . '_alias, t2.weight FROM ' . TABLE_SHOP_MAIN . '_rows as t1 INNER JOIN ' . TABLE_SHOP_MAIN . '_block AS t2 ON t1.id = t2.id WHERE t2.bid= ' . $bid . ' AND t1.inhome=1 ORDER BY t2.weight ASC';
...
function nv_show_custom_form($is_edit, $form, $array_custom)
{
    ...
    $idtemplate = $db->query('SELECT id FROM ' . TABLE_SHOP_MAIN . '_template WHERE alias = "' . preg_replace("/[\_]/", "-", $form) . '"')->fetchColumn();
    if ($idtemplate) {
        $array_tmp = array( );
        $result = $db->query('SELECT * FROM ' . TABLE_SHOP_MAIN . '_field');
        while ($row = $result->fetch()) {
...
function nv_create_form_file($array_template_id)
{
    ...
    foreach ($array_template_id as $templateids_i) {
        $array_views = array();
        $result = $db->query("SELECT fid, field, field_type, listtemplate FROM " . TABLE_SHOP_MAIN . "_field");
