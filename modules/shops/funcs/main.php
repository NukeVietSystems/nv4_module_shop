...
if (empty($contents)) {
    ...
    if ($pro_config['home_view'] == 'view_home_all') {
        // Fetch Limit
        $db->sqlreset()->select('COUNT(*)')->from(TABLE_SHOP_MAIN . '_rows t1')->where('t1.inhome=1 AND t1.status =1 AND idsite =' . IDSITE .'');
        ...
        foreach ($global_array_shops_cat as $catid_i => $array_info_i) {
            if ($array_info_i['parentid'] == 0 and $array_info_i['inhome'] != 0) {
                ...
                // Fetch Limit
                $db->sqlreset()->select('COUNT(*)')->from(TABLE_SHOP_MAIN . '_rows t1')->where('listcatid IN (' . implode(',', $array_cat) . ') AND inhome=1 AND status =1');
        ...
        foreach ($global_array_group as $groupid_i => $array_info_i) {
            if ($array_info_i['parentid'] == 0 and $array_info_i['inhome'] != 0) {
                ...
                $db->sqlreset()->select('DISTINCT id')
                    ->from(TABLE_SHOP_MAIN . '_rows t1')
                    ->join('INNER JOIN ' . TABLE_SHOP_MAIN . '_group_items t3 ON t3.pro_id = t1.id')
        ...
        while (list($id, $listcatid, $mcatid, $publtime, $title, $alias, $hometext, $homeimgalt, $homeimgfile, $homeimgthumb, $product_code, $product_number, $product_price, $money_unit, $discount_id, $showprice, $gift_content, $gift_from, $gift_to, $idsite) = $result->fetch(3)) {
            if ($homeimgthumb == 1) {
                //image thumb

                $thumb = NV_BASE_SITEURL . NV_ASSETS_DIR . '/site/' . $global_array_site[$idsite]['domain'] . '/' . $module_upload . '/' . $homeimgfile;
            } elseif ($homeimgthumb == 2) {
                //image file

                $thumb = NV_BASE_SITEURL . SYSTEM_UPLOADS_DIR . '/' . $global_array_site[$idsite]['domain'] . '/' . $module_upload . '/' . $homeimgfile;
            } elseif ($homeimgthumb == 3) {
                //image url
            ...
            $data_content[] = array(
                ...
                'money_unit' => $money_unit,
                'showprice' => $showprice,
                'newday' => $global_array_shops_cat[$listcatids]['newday'],
                'gift_content' => $gift_content,
                'gift_from' => $gift_from,
                'gift_to' => $gift_to,
                'link_pro' => NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $global_array_shops_cat[$listcatids]['alias'] . '/' . $alias . $global_config['rewrite_exturl'],
                'link_order' => NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=setcart&amp;id=' . $id
            );
        }
