...
if ($nv_Request->isset_request('check_quantity', 'post')) {
    ...
    $quantity = $db->query('SELECT quantity FROM ' . TABLE_SHOP_MAIN . '_group_quantity WHERE pro_id = ' . $id_pro . ' AND listgroup="' . implode(',', $listid) . '"')->fetchColumn();
    if (empty($quantity)) {
        $sum = 0;
        $count = 0;
        ...
        $result = $db->query('SELECT listgroup, quantity FROM ' . TABLE_SHOP_MAIN . '_group_quantity WHERE pro_id = ' . $id_pro);
...
$sql = $db->query('SELECT * FROM ' . TABLE_SHOP_MAIN . '_rows WHERE ' . NV_LANG_DATA . '_alias = ' . $db->quote($alias_url) . ' AND status=1');
...
if ($global_array_shops_cat[$data_content['listcatid']]['form'] != '') {
    ...
    if ($idtemplate) {
        $listfield = array();
        $array_tmp = array();
        $result = $db->query('SELECT * FROM ' . TABLE_SHOP_MAIN . '_field ORDER BY weight');
        ...
        if (!empty($listfield)) {
            $result = $db->query('SELECT t1.field_value, t2.field FROM ' . $db_config['prefix'] . "_" . $module_data . "_field_value_" . NV_LANG_DATA . ' t1 INNER JOIN ' . TABLE_SHOP_MAIN . '_field t2 WHERE t1.field_id=t2.fid AND t1.rows_id=' . $id);
...
if (nv_user_in_groups($global_array_shops_cat[$catid]['groups_view'])) {
    ...
    // Lay don vi san pham
    $sql = $db->query('SELECT * FROM ' . TABLE_SHOP_MAIN . '_units WHERE id = ' . $data_content['product_unit']);
    $data_unit = $sql->fetch();
    $data_unit['title'] = $data_unit[NV_LANG_DATA . '_title'];
    
    // Hien thi tabs
    $sql = 'SELECT * FROM ' . TABLE_SHOP_MAIN . '_tabs where active=1 ORDER BY weight ASC';
    ...
    if (!empty($data_content['tabs'])) {
        ...
        if ($pro_config['download_active']) {
            $result = $db->query('SELECT id, ' . NV_LANG_DATA . '_title title, ' . NV_LANG_DATA . '_description description, path, filesize, extension, download_groups FROM ' . TABLE_SHOP_MAIN . '_files WHERE id IN (SELECT id_files FROM ' . TABLE_SHOP_MAIN . '_files_rows WHERE id_rows=' . $data_content['id'] . ')');
            while ($row = $result->fetch()) {
    ...
    // Danh gia - Phan hoi
    $rating_total = 0;
    $result = $db->query('SELECT rating FROM ' . TABLE_SHOP_MAIN . '_review WHERE product_id = ' . $data_content['id'] . ' AND status=1');
    ...
    $array_keyword = array();
    $key_words = array();
    $_query = $db->query('SELECT a1.keyword keyword, a2.alias alias FROM ' . TABLE_SHOP_MAIN . '_tags_id_' . NV_LANG_DATA . ' a1 INNER JOIN ' . TABLE_SHOP_MAIN . '_tags_' . NV_LANG_DATA . ' a2 ON a1.tid=a2.tid WHERE a1.id=' . $data_content['id']);
