...
if (IDSITE == 0 ){
	...
	$sql = 'SELECT * FROM ' . $db_config['prefix'] . '_site ORDER BY idsite ASC';
	$result = $db->query($sql);
	$list_idsite='0';
	while ($row = $result->fetch()) {
		$global_array_site[$row['idsite']] = array(
				'idsite' => $row['idsite'],
				'domain' => $row['domain']
			);
		$list_idsite.=','.$row['idsite'];
	}
}
// Main Categories
$sql = 'SELECT catid, parentid, lev, ' . NV_LANG_DATA . '_title AS title, ' . NV_LANG_DATA . '_title_custom AS title_custom, ' . NV_LANG_DATA . '_alias AS alias, viewcat, numsubcat, subcatid, newday, typeprice, form, group_price, viewdescriptionhtml, numlinks, ' . NV_LANG_DATA . '_description AS description, ' . NV_LANG_DATA . '_descriptionhtml AS descriptionhtml, inhome, ' . NV_LANG_DATA . '_keywords AS keywords, ' . NV_LANG_DATA . '_tag_description AS tag_description, groups_view, cat_allow_point, cat_number_point, cat_number_product, image, idsite FROM ' . TABLE_SHOP_MAIN . '_catalogs WHERE idsite = 0 ORDER BY sort ASC';
$global_array_mshops_cat = array();
$result = $db->query($sql);
while ($row = $result->fetch()) {
	$global_array_mshops_cat [$row['catid']] = $row;
}

// Categories
$sql = 'SELECT catid, parentid, lev, ' . NV_LANG_DATA . '_title AS title, ' . NV_LANG_DATA . '_title_custom AS title_custom, ' . NV_LANG_DATA . '_alias AS alias, viewcat, numsubcat, subcatid, newday, typeprice, form, group_price, viewdescriptionhtml, numlinks, ' . NV_LANG_DATA . '_description AS description, ' . NV_LANG_DATA . '_descriptionhtml AS descriptionhtml, inhome, ' . NV_LANG_DATA . '_keywords AS keywords, ' . NV_LANG_DATA . '_tag_description AS tag_description, groups_view, cat_allow_point, cat_number_point, cat_number_product, image, idsite FROM ' . TABLE_SHOP_MAIN . '_catalogs  ORDER BY idsite ASC, sort ASC';
$global_array_shops_cat = $nv_Cache->db($sql, 'catid', $module_name);

// Groups
$sql = 'SELECT groupid, parentid, lev, ' . NV_LANG_DATA . '_title AS title, ' . NV_LANG_DATA . '_alias AS alias, viewgroup, numsubgroup, subgroupid, ' . NV_LANG_DATA . '_description AS description, inhome, indetail, in_order, ' . NV_LANG_DATA . '_keywords AS keywords, numpro, image, is_require FROM ' . TABLE_SHOP_MAIN . '_group ORDER BY sort ASC';
$global_array_group = $nv_Cache->db($sql, 'groupid', $module_name);

// Lay ty gia ngoai te
$sql = 'SELECT code, currency,symbol, exchange, round, number_format FROM ' . TABLE_SHOP_MAIN . '_money_' . NV_LANG_DATA;
...
$sql = 'SELECT code, title, exchange, round FROM ' . TABLE_SHOP_MAIN . '_weight_' . NV_LANG_DATA;
...
// Lay dia diem
$sql = 'SELECT * FROM ' . TABLE_SHOP_MAIN . '_location ORDER BY sort ASC';
$array_location = $nv_Cache->db($sql, 'id', $module_name);

// Lay nha van chuyen
$sql = 'SELECT * FROM ' . TABLE_SHOP_MAIN . '_carrier WHERE status = 1 ORDER BY weight ASC';
$array_carrier = $nv_Cache->db($sql, 'id', $module_name);

// Lay cua hang
$sql = 'SELECT * FROM ' . TABLE_SHOP_MAIN . '_shops WHERE status = 1 ORDER BY weight ASC';
$array_shops = $nv_Cache->db($sql, 'id', $module_name);

// Lay Giam Gia
$sql = 'SELECT did, title, begin_time, end_time, config FROM ' . TABLE_SHOP_MAIN . '_discounts';
$cache_file = NV_LANG_DATA . '_' . md5($sql) . '_' . NV_CACHE_PREFIX . '.cache';
if (($cache = $nv_Cache->getItem($module_name, $cache_file)) != false) {
...
function nv_get_price($pro_id, $currency_convert, $number = 1, $per_pro = false, $module = '')
{
    ...
    $product = $db->query('SELECT listcatid, mcatid, product_price, money_unit, price_config, discount_id, idsite FROM ' . TABLE_SHOP_MAIN . '_rows WHERE id = ' . $pro_id)->fetch();
...
function nv_shipping_price($weight, $weight_unit, $location_id, $shops_id, $carrier_id)
{
    ...
    $sql = 'SELECT config_id FROM ' . TABLE_SHOP_MAIN . '_shops_carrier WHERE shops_id = ' . $shops_id . ' AND carrier_id = ' . $carrier_id;
    $result = $db->query($sql);
    list($config_id) = $result->fetch(3);

    if ($config_id) {
        $sql = 'SELECT iid FROM ' . TABLE_SHOP_MAIN . '_carrier_config_location WHERE cid = ' . $config_id . ' AND lid = ' . $location_id;
        $result = $db->query($sql);
        list($iid) = $result->fetch(3);

        if ($iid) {
            // Weight config
            $sql = 'SELECT * FROM ' . TABLE_SHOP_MAIN . '_carrier_config_weight WHERE iid = ' . $iid . ' ORDER BY weight';
            $result = $db->query($sql);
