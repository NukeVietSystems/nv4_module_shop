...
if ($nv_Request->isset_request('ajax_action', 'post')) {
    ...
    $content = 'NO_' . $id;
    if ($new_vid > 0) {
        $sql = 'SELECT id FROM ' . TABLE_SHOP_MAIN . '_carrier_config_items WHERE id!=' . $id . ' and cid=' . $cid . ' ORDER BY weight ASC';
        $result = $db->query($sql);
        $weight = 0;
        while ($row = $result->fetch()) {
            ...
            $sql = 'UPDATE ' . TABLE_SHOP_MAIN . '_carrier_config_items SET weight=' . $weight . ' WHERE id=' . $row['id'] . ' AND cid=' . $cid;
            $db->query($sql);
        }
        $sql = 'UPDATE ' . TABLE_SHOP_MAIN . '_carrier_config_items SET weight=' . $new_vid . ' WHERE id=' . $id . ' AND cid=' . $cid;
        $db->query($sql);
        $content = 'OK_' . $id;
    }
...
if ($nv_Request->isset_request('delete_id', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
    ...
    if ($id > 0 and $delete_checkss == md5($id . NV_CACHE_PREFIX . $client_info['session_id'])) {
        $weight = 0;
        $sql = 'SELECT weight FROM ' . TABLE_SHOP_MAIN . '_carrier_config_items WHERE id =' . $db->quote($id);
        $result = $db->query($sql);
        list($weight) = $result->fetch(3);

        $db->query('DELETE FROM ' . TABLE_SHOP_MAIN . '_carrier_config_items  WHERE id = ' . $db->quote($id));

        // Xoa bang carrier_config_location
        $db->query('DELETE FROM ' . TABLE_SHOP_MAIN . '_carrier_config_location WHERE iid = ' . $id);

        // Xoa bang carrier_config_weight
        $db->query('DELETE FROM ' . TABLE_SHOP_MAIN . '_carrier_config_weight WHERE iid = ' . $id);

        if ($weight > 0) {
            $sql = 'SELECT id, weight FROM ' . TABLE_SHOP_MAIN . '_carrier_config_items WHERE weight >' . $weight;
            $result = $db->query($sql);
            while (list($id, $weight) = $result->fetch(3)) {
                $weight--;
                $db->query('UPDATE ' . TABLE_SHOP_MAIN . '_carrier_config_items SET weight=' . $weight . ' WHERE id=' . intval($id));
            }
        }
        $nv_Cache->delMod($module_name);
...
if ($row['id'] > 0) {
    $row = $db->query('SELECT * FROM ' . TABLE_SHOP_MAIN . '_carrier_config_items WHERE id=' . $row['id'])->fetch();
    if (empty($row)) {
    ...
    $result = $db->query('SELECT * FROM ' . TABLE_SHOP_MAIN . '_carrier_config_weight WHERE iid=' . $row['id'] . ' ORDER BY weight');
    if ($result) {
    ...
    $result = $db->query('SELECT lid FROM ' . TABLE_SHOP_MAIN . '_carrier_config_location WHERE iid=' . $row['id']);
    if ($result) {
...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    if (empty($error)) {
        try {
            if (empty($row['id'])) {
                $sql = 'INSERT INTO ' . TABLE_SHOP_MAIN . '_carrier_config_items (cid, title, description, weight, add_time) VALUES (:cid, :title, :description, :weight, ' . NV_CURRENTTIME . ' )';

                $weight = $db->query('SELECT max(weight) FROM ' . TABLE_SHOP_MAIN . '_carrier_config_items WHERE cid=' . $row['cid'])->fetchColumn();
                ...
                $stmt = $db->prepare('UPDATE ' . TABLE_SHOP_MAIN . '_carrier_config_items SET cid = :cid, title = :title, description = :description WHERE id=' . $row['id']);
            ...
            if ($exc or $insert_id) {
                ...
                if (!empty($row['config_weight'])) {
                    if (!empty($row['id'])) {
                        $db->query('DELETE FROM ' . TABLE_SHOP_MAIN . '_carrier_config_weight WHERE iid=' . $row['id']);
                    }
                    foreach ($row['config_weight'] as $config_weight) {
                        ...
                        if (empty($row['id'])) {
                            $db->query('INSERT INTO ' . TABLE_SHOP_MAIN . '_carrier_config_weight (iid, weight, weight_unit, carrier_price, carrier_price_unit) VALUES (' . $insert_id . ', ' . $config_weight['weight'] . ', ' . $db->quote($config_weight['weight_unit']) . ', ' . $config_weight['carrier_price'] . ', ' . $db->quote($config_weight['carrier_price_unit']) . ')');
                        } else {
                            $db->query('INSERT INTO ' . TABLE_SHOP_MAIN . '_carrier_config_weight (iid, weight, weight_unit, carrier_price, carrier_price_unit) VALUES (' . $row['id'] . ', ' . $config_weight['weight'] . ', ' . $db->quote($config_weight['weight_unit']) . ', ' . $config_weight['carrier_price'] . ', ' . $db->quote($config_weight['carrier_price_unit']) . ')');
                        }
                    }
                }
                ...
                if ($row['config_location'] != $config_location_old) {
                    if (!empty($row['id'])) {
                        $db->query('DELETE FROM ' . TABLE_SHOP_MAIN . '_carrier_config_location WHERE iid=' . $row['id']);
                    }
                    foreach ($row['config_location'] as $config_location_id) {
                        if (empty($row['id'])) {
                            $db->query('INSERT INTO ' . TABLE_SHOP_MAIN . '_carrier_config_location ( cid, iid, lid ) VALUES ( ' . $row['cid'] . ' , ' . $insert_id . ', ' . $config_location_id . ')');
                        } else {
                            $db->query('INSERT INTO ' . TABLE_SHOP_MAIN . '_carrier_config_location ( cid, iid, lid ) VALUES ( ' . $row['cid'] . ', ' . $row['id'] . ', ' . $config_location_id . ')');
                        }
                    }
                }
...
if (!$nv_Request->isset_request('id', 'post,get')) {
    ...
    $db->sqlreset()->select('COUNT(*)')->from('' . TABLE_SHOP_MAIN . '_carrier_config_items')->where('cid=' . $row['cid']);
...
// Lay dia diem
if (!empty($row['id'])) {
    $sql = "SELECT id, title, lev FROM " . TABLE_SHOP_MAIN . "_location WHERE id NOT IN ( SELECT lid FROM " . $db_config['prefix'] . "_" . $module_data . "_carrier_config_location WHERE cid = " . $row['cid'] . " ) OR id IN ( SELECT lid FROM " . TABLE_SHOP_MAIN . "_carrier_config_location WHERE iid = " . $row['id'] . " ) ORDER BY sort ASC";
} else {
    $sql = "SELECT id, title, lev FROM " . TABLE_SHOP_MAIN . "_location WHERE id NOT IN ( SELECT lid FROM " . $db_config['prefix'] . "_" . $module_data . "_carrier_config_location WHERE cid = " . $row['cid'] . " ) ORDER BY sort ASC";
}
$result = $db->query($sql);
...
$_sql = 'SELECT id, title FROM ' . TABLE_SHOP_MAIN . '_carrier_config ORDER BY id DESC';
