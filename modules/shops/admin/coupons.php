...
if ($nv_Request->isset_request('delete_id', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
    ...
    if ($id > 0 and $delete_checkss == md5($id . NV_CACHE_PREFIX . $client_info['session_id'])) {
        $db->query('DELETE FROM ' . TABLE_SHOP_MAIN . '_coupons WHERE id = ' . $db->quote($id));
...
if ($nv_Request->isset_request('get_product', 'get')) {
    ...
    $db->sqlreset()->select('id, ' . NV_LANG_DATA . '_title')->from(TABLE_SHOP_MAIN . '_rows')->where(NV_LANG_DATA . '_title LIKE :title OR ' . NV_LANG_DATA . '_alias LIKE :alias')->order(NV_LANG_DATA . '_title ASC')->limit(50);
...
if ($row['id'] > 0) {
    $result = $db->query('SELECT pid FROM ' . TABLE_SHOP_MAIN . '_coupons_product WHERE cid=' . $row['id']);
...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    if (empty($error)) {
        try {
            if (empty($row['id'])) {
                $sql = 'INSERT INTO ' . TABLE_SHOP_MAIN . '_coupons (title, code, type, discount, total_amount, date_start, date_end, uses_per_coupon, date_added, status) VALUES (:title, :code, :type, :discount, :total_amount, :date_start, :date_end, :uses_per_coupon, ' . NV_CURRENTTIME . ', 1)';
                $data_insert = array( );
                ...
                $stmt = $db->prepare('UPDATE ' . TABLE_SHOP_MAIN . '_coupons SET title = :title, code = :code, type = :type, discount = :discount, total_amount = :total_amount, date_start = :date_start, date_end = :date_end, uses_per_coupon = :uses_per_coupon WHERE id=' . $row['id']);
            ...
            if ($exc or $insert_id > 0) {
                ...
                if (empty($row['id']) and !empty($row['product'])) {
                    foreach ($row['product'] as $pid) {
                        $db->query('INSERT INTO ' . TABLE_SHOP_MAIN . '_coupons_product VALUES( ' . $insert_id . ', ' . $pid . ')');
                    }
                } else {
                    if ($product_old != $row['product']) {
                        $db->query('DELETE FROM ' . TABLE_SHOP_MAIN . '_coupons_product WHERE cid=' . $row['id']);
                        foreach ($row['product'] as $pid) {
                            if (!empty($pid)) {
                                $db->query('INSERT INTO ' . TABLE_SHOP_MAIN . '_coupons_product VALUES(' . $row['id'] . ', ' . $pid . ')');
                            }
                        }
                    }
                }
    ...
    $row = $db->query('SELECT * FROM ' . TABLE_SHOP_MAIN . '_coupons WHERE id=' . $row['id'])->fetch();
...
if (!$nv_Request->isset_request('id', 'post,get')) {
    ...
    $db->sqlreset()->select('COUNT(*)')->from(TABLE_SHOP_MAIN . '_coupons');
...
if (!empty($row['product'])) {
    $array_pro = array( );
    $result = $db->query('SELECT id, ' . NV_LANG_DATA . '_title FROM ' . TABLE_SHOP_MAIN . '_rows WHERE id IN (' . implode(',', $row['product']) . ')');
