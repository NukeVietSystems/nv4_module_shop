...
$sql = 'SELECT bid, ' . NV_LANG_DATA . '_title FROM ' . TABLE_SHOP_MAIN . '_block_cat ORDER BY weight ASC';
$result = $db->query($sql);
...
if ($listid == '') {
    $db->sqlreset()->select('id, ' . NV_LANG_DATA . '_title')->from(TABLE_SHOP_MAIN . '_rows')->where('inhome=1 AND id NOT IN(SELECT id FROM ' . TABLE_SHOP_MAIN . '_block WHERE bid=' . $bid . ')')->order('id DESC')->limit(20);
    $sql = $db->sql();
} else {
    $id_array = array_map('intval', explode(',', $listid));
    $sql = 'SELECT id, ' . NV_LANG_DATA . '_title FROM ' . TABLE_SHOP_MAIN . '_rows WHERE inhome=1 AND id IN (' . implode(',', $id_array) . ') ORDER BY id DESC';
}
