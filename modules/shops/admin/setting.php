...
if ($savesetting == 1) {
    ...
    if ($error == '') {
        ...
        $sql = "UPDATE " . TABLE_SHOP_MAIN . "_money_" . NV_LANG_DATA . " SET exchange = '1' WHERE id = " . $mid;
        $db->query($sql);
...
if ($data['active_payment'] == '1') {
    $sql = "SELECT * FROM " . TABLE_SHOP_MAIN . "_payment ORDER BY weight ASC";
    $result = $db->query($sql);
...
// Tien te
$result = $db->query("SELECT code, currency FROM " . TABLE_SHOP_MAIN . "_money_" . NV_LANG_DATA . " ORDER BY code DESC");
...
// Don vi khoi luong
$result = $db->query("SELECT code, title FROM " . TABLE_SHOP_MAIN . "_weight_" . NV_LANG_DATA . " ORDER BY code DESC");
