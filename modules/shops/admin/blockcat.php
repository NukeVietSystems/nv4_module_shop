...
$table_name = TABLE_SHOP_MAIN . '_block_cat';
...
if (! empty($savecat)) {
    ...
    if (empty($data['title'])) {
        ...
        if ($data['bid'] == 0) {
            $stmt = $db->prepare('SELECT bid FROM ' . TABLE_SHOP_MAIN . '_block_cat WHERE ' . NV_LANG_DATA . '_alias= :alias');
            $stmt->bindParam(':alias', $data['alias'], PDO::PARAM_STR);
            $stmt->execute();
            if ($stmt->rowCount()) {
                $error = $lang_module['block_error_alias'];
            } else {
                $weight = $db->query('SELECT max(weight) FROM ' . TABLE_SHOP_MAIN . '_block_cat')->fetchColumn();
            ...
            $stmt = $db->prepare('SELECT bid FROM ' . TABLE_SHOP_MAIN . '_block_cat WHERE ' . NV_LANG_DATA . '_alias= :alias AND bid!=' . $data['bid']);
            $stmt->bindParam(':alias', $data['alias'], PDO::PARAM_STR);
            $stmt->execute();
            if ($stmt->rowCount()) {
                $error = $lang_module['block_error_alias'];
            } else {
                $stmt = $db->prepare('UPDATE ' . TABLE_SHOP_MAIN . '_block_cat SET ' . NV_LANG_DATA . '_title= :title, ' . NV_LANG_DATA . '_alias = :alias, ' . NV_LANG_DATA . '_description= :description, ' . NV_LANG_DATA . '_keywords= :keywords, image = :image, edit_time=' . NV_CURRENTTIME . ' WHERE bid =' . $data['bid']);
...
if ($data['bid'] > 0) {
    list($data['bid'], $data['title'], $data['alias'], $data['description'], $data['keywords'], $data['image']) = $db->query('SELECT bid, ' . NV_LANG_DATA . '_title, ' . NV_LANG_DATA . '_alias, ' . NV_LANG_DATA . '_description, ' . NV_LANG_DATA . '_keywords, image FROM ' . TABLE_SHOP_MAIN . '_block_cat where bid=' . $data['bid'])->fetch(3);
