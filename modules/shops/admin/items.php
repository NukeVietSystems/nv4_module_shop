...
$from = TABLE_SHOP_MAIN . '_rows AS a LEFT JOIN ' . NV_USERS_GLOBALTABLE . ' AS b ON a.user_id=b.userid ';
...
$db->sqlreset()->select('id, listcatid, mcatid, user_id, homeimgfile, homeimgthumb, ' . NV_LANG_DATA . '_title, ' . NV_LANG_DATA . '_alias, hitstotal, status, edittime, publtime, exptime, product_number, product_price, money_unit, product_unit, a.idsite, num_sell, username')->from($from)->order($ord_sql)->limit($per_page)->offset(($page - 1) * $per_page);
//die($db->sql());
...
while (list($id, $listcatid, $mcatid, $admin_id, $homeimgfile, $homeimgthumb, $title, $alias, $hitstotal, $status, $edittime, $publtime, $exptime, $product_number, $product_price, $money_unit, $product_unit, $idsite, $num_sell, $username) = $result->fetch(3)) {
    ...
    if ($homeimgthumb == 1) {
        ...
        //image file
		if(IDSITE==0){
			$imghome = $thumb = NV_BASE_SITEURL . SYSTEM_UPLOADS_DIR . '/' . $global_array_site[$idsite]['domain'] . '/' . $module_upload . '/' . $homeimgfile;
		}else{
