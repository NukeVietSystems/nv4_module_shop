...
$table_name = TABLE_SHOP_MAIN . '_orders';
...
$result = $db->query('SELECT * FROM ' . TABLE_SHOP_MAIN . '_orders_id WHERE order_id=' . $order_id);
while ($row = $result->fetch()) {
    $listid[] = $row['proid'];
    $listnum[] = $row['num'];
    $listprice[] = $row['price'];

    $result_group = $db->query('SELECT group_id FROM ' . TABLE_SHOP_MAIN . '_orders_id_group WHERE order_i=' . $row['id']);
    $group = array();
...
foreach ($listid as $id) {
    $sql = 'SELECT t1.id, t1.listcatid, t1.product_code, t1.publtime, t1.' . NV_LANG_DATA . '_title, t1.' . NV_LANG_DATA . '_alias, t1.product_price,t2.' . NV_LANG_DATA . '_title FROM ' . TABLE_SHOP_MAIN . '_units AS t2, ' . TABLE_SHOP_MAIN . '_rows AS t1 WHERE t1.product_unit = t2.id AND t1.id =' . $id . ' AND t1.status =1 AND t1.publtime < ' . NV_CURRENTTIME . ' AND (t1.exptime=0 OR t1.exptime>' . NV_CURRENTTIME . ')';
    $result = $db->query($sql);
    if ($result->rowCount()) {
...
if ($save == 1) {
    ...
    if ($action == 'unpay') {
        $db->query('DELETE FROM ' . TABLE_SHOP_MAIN . '_transaction WHERE transaction_id = ' . $data_content['transaction_id']);
