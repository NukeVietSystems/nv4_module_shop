...
if ($nv_Request->isset_request('coupons_history', 'post,get')) {
    ...
    $db->sqlreset()
      ->select('COUNT(*)')
      ->from(TABLE_SHOP_MAIN . '_coupons_history t1')
      ->join('INNER JOIN ' . TABLE_SHOP_MAIN . '_orders t2 ON t1.order_id = t2.order_id')
      ->where('cid=' . $id);
...
$array_data = array();
$result = $db->query('SELECT * FROM ' . TABLE_SHOP_MAIN . '_coupons WHERE id = ' . $id);
$array_data = $result->fetch();

$result = $db->query('SELECT * FROM ' . TABLE_SHOP_MAIN . '_coupons_product WHERE cid = ' . $array_data['id']);
