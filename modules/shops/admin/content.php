...
$table_name = TABLE_SHOP_MAIN . '_rows';
$array_block_cat_module = array();
$id_block_content = array();
$array_custom = $array_custom_old = array();
$custom = array();

$sql = 'SELECT bid, adddefault, ' . NV_LANG_DATA . '_title FROM ' . TABLE_SHOP_MAIN . '_block_cat ORDER BY weight ASC';
$result = $db->query($sql);
...
$group_id_old = array( );
if ($rowcontent['id'] > 0) {
    $rowcontent['listcatid'] = $db->query("SELECT listcatid FROM " . TABLE_SHOP_MAIN . "_rows where id=" . $rowcontent['id'])->fetchColumn();
    ...
    $_query = $db->query('SELECT tid, keyword FROM ' . TABLE_SHOP_MAIN . '_tags_id_' . NV_LANG_DATA . ' WHERE id=' . $rowcontent['id'] . ' ORDER BY keyword ASC');
    ...
    // Old files
    $result = $db->query("SELECT id_files FROM " . TABLE_SHOP_MAIN . "_files_rows WHERE id_rows=" . $rowcontent['id']);
    ...
    // Custom fields
    $idtemplate = $db->query('SELECT id FROM ' . TABLE_SHOP_MAIN . '_template WHERE alias = "' . preg_replace("/[\_]/", "-", $global_array_shops_cat[$rowcontent['listcatid']]['form']) . '"')->fetchColumn();
    if ($idtemplate) {
        $result = $db->query("SELECT * FROM " . TABLE_SHOP_MAIN . "_field_value_" . NV_LANG_DATA . " WHERE rows_id=" . $rowcontent['id']);
...
if ($nv_Request->get_int('save', 'post') == 1) {
    ...
    $rowcontent['mcatid'] = $nv_Request->get_int('mcatid', 'post', 0);
    ...
    if (!empty($rowcontent['alias'])) {
        $stmt = $db->prepare('SELECT COUNT(*) FROM ' . TABLE_SHOP_MAIN . '_rows WHERE id !=' . $rowcontent['id'] . ' AND '  . NV_LANG_DATA . '_alias = :alias');
        $stmt->bindParam(':alias', $rowcontent['alias'], PDO::PARAM_STR);
        $stmt->execute();
        if ($stmt->fetchColumn()) {
            $rows_id = $rowcontent['id'];
            if ($rows_id == 0) {
                $rows_id = $db->query('SELECT MAX(id) FROM ' . TABLE_SHOP_MAIN . '_rows')->fetchColumn();
    ...
    if (!empty($rowcontent['product_code'])) {
        $stmt = $db->prepare('SELECT id FROM ' . TABLE_SHOP_MAIN . '_rows WHERE product_code= :product_code AND id!=' . $rowcontent['id']);
        $stmt->bindParam(':product_code', $rowcontent['product_code'], PDO::PARAM_STR);
        $stmt->execute();
        $id_err = $stmt->rowCount();

        $stmt = $db->prepare('SELECT id FROM ' . TABLE_SHOP_MAIN . '_rows WHERE product_code= :product_code');
        $stmt->bindParam(':product_code', $rowcontent['product_code'], PDO::PARAM_STR);
        $stmt->execute();
    ...
    } elseif (empty($rowcontent['mcatid'])) {
        $error = $lang_module['error_mcat'];
    ...
    $group_cat = array();
    $result = $db->query('SELECT groupid FROM ' . TABLE_SHOP_MAIN . '_group_cateid WHERE cateid = ' . $rowcontent['listcatid']);
    if ($result->rowCount() > 0) {
    ...
    if (empty($error)) {
        ...
        if ($rowcontent['keywords'] == '') {
            ...
            $sth = $db->prepare('SELECT COUNT(*) FROM ' . TABLE_SHOP_MAIN . '_tags_id_' . NV_LANG_DATA . ' where keyword = :keyword');
        ...
        if ($rowcontent['id'] == 0 or $is_copy) {
            ...
            if ($rowcontent['id'] > 0) {
                ...
                if ($global_array_shops_cat[$rowcontent['listcatid']]['form'] != '') {
                    foreach ($array_custom as $field_id => $value) {
                        $sth = $db->prepare('INSERT INTO ' . TABLE_SHOP_MAIN . '_field_value_' . NV_LANG_DATA . '(rows_id, field_id, field_value) VALUES (:rows_id, :field_id, :field_value)');
                ...
                if (!empty($rowcontent['group_id'])) {
                    $stmt = $db->prepare('INSERT INTO ' . TABLE_SHOP_MAIN . '_group_items VALUES(' . $rowcontent['id'] . ', :group_id)');
                ...
                if (!empty($pro_config['format_code_id']) and empty($rowcontent['product_code'])) {
                    ...
                    $stmt = $db->prepare('UPDATE ' . TABLE_SHOP_MAIN . '_rows SET product_code= :product_code WHERE id=' . $rowcontent['id']);
                    $stmt->bindParam(':product_code', $auto_product_code, PDO::PARAM_STR);
                    $stmt->execute();
                }
                ...
                if ($pro_config['download_active']) {
                    if (!empty($rowcontent['files'])) {
                        foreach ($rowcontent['files'] as $id_files) {
                            $db->query('INSERT INTO ' . TABLE_SHOP_MAIN . '_files_rows (id_rows, id_files, download_hits) VALUES (' . $rowcontent['id'] . ', ' . $id_files .', 0)');
                        }
                    }
                }
            ...
            $rowcontent_old = $db->query('SELECT * FROM ' . TABLE_SHOP_MAIN . '_rows where id=' . $rowcontent['id'])->fetch();
            ...
            $stmt = $db->prepare("UPDATE " . TABLE_SHOP_MAIN . "_rows SET
			 listcatid= :listcatid,mcatid= :mcatid,
            ...
            $stmt->bindParam(':mcatid', $rowcontent['mcatid'], PDO::PARAM_STR);
            $stmt->bindParam(':product_code', $rowcontent['product_code'], PDO::PARAM_STR);
            ...
            if ($stmt->execute()) {
                ...
                if ($group_id_old != $rowcontent['group_id']) {
                    $sql = 'DELETE FROM ' . TABLE_SHOP_MAIN . '_group_items WHERE pro_id = ' . $rowcontent['id'];
                    $db->query($sql);

                    if (!empty($rowcontent['group_id'])) {
                        $stmt = $db->prepare('INSERT INTO ' . TABLE_SHOP_MAIN . '_group_items(pro_id, group_id) VALUES(' . $rowcontent['id'] . ', :group_id)');
                        foreach ($rowcontent['group_id'] as $group_id_i) {
                            $stmt->bindParam(':group_id', $group_id_i, PDO::PARAM_STR);
                            $stmt->execute();
                        }
                    ...
                    if ($rowcontent['files'] != $rowcontent['files_old']) {
                        foreach ($rowcontent['files'] as $id_files) {
                            if (!in_array($id_files, $rowcontent['files_old'])) {
                                $db->query('INSERT INTO ' . TABLE_SHOP_MAIN . '_files_rows (id_rows, id_files) VALUES (' . $rowcontent['id'] .', ' . $id_files . ')');
                            }
                        }

                        foreach ($rowcontent['files_old'] as $id_files_old) {
                            if (!in_array($id_files_old, $rowcontent['files'])) {
                                $db->query('DELETE FROM ' . TABLE_SHOP_MAIN . '_files_rows WHERE id_files = ' . $id_files_old);
                            }
                        }
                    }
                }
                ...
                if ($global_array_shops_cat[$rowcontent_old['listcatid']]['form'] != '') {
                    foreach ($array_custom as $field_id => $value) {
                        $count = $db->query('SELECT COUNT(*) FROM ' . TABLE_SHOP_MAIN . '_field_value_' . NV_LANG_DATA . ' WHERE rows_id=' . $rowcontent['id'] . ' AND field_id=' . $field_id)->fetchColumn();
                        if ($count > 0) {
                            $sth = $db->prepare('UPDATE ' . TABLE_SHOP_MAIN . '_field_value_' . NV_LANG_DATA . ' SET field_value = :field_value WHERE rows_id = :rows_id AND field_id = :field_id');
                        } else {
                            $sth = $db->prepare('INSERT INTO ' . TABLE_SHOP_MAIN . '_field_value_' . NV_LANG_DATA . '(rows_id, field_id, field_value) VALUES (:rows_id, :field_id, :field_value)');
                        }
                        $sth->bindParam(':rows_id', $rowcontent['id'], PDO::PARAM_INT);
        ...
        if ($error == '') {
            $db->query('DELETE FROM ' . TABLE_SHOP_MAIN . '_block WHERE id = ' . $rowcontent['id']);

            foreach ($id_block_content as $bid_i) {
                $db->query("INSERT INTO " . TABLE_SHOP_MAIN . "_block (bid, id, weight) VALUES ('" . $bid_i . "', '" . $rowcontent['id'] . "', '0')");
            }
            ...
            if ($rowcontent['keywords'] != $rowcontent['keywords_old']) {
                ...
                foreach ($keywords as $keyword) {
                    if (!in_array($keyword, $array_keywords_old)) {
                        ...
                        $sth = $db->prepare('SELECT tid, alias, description, keywords FROM ' . TABLE_SHOP_MAIN . '_tags_' . NV_LANG_DATA . ' where alias= :alias OR FIND_IN_SET(:keyword, keywords)>0');
                        ...
                        if (empty($tid)) {
                            ...
                            $tid = $db->insert_id("INSERT INTO " . TABLE_SHOP_MAIN . "_tags_" . NV_LANG_DATA . " (numpro, alias, description, image, keywords) VALUES (1, :alias, '', '', :keyword)", "tid", $array_insert);
                        } else {
                            if ($alias != $alias_i) {
                                if (!empty($keywords_i)) {
                                ...
                                if ($keywords_i != $keywords_i2) {
                                    $sth = $db->prepare('UPDATE ' . TABLE_SHOP_MAIN . '_tags_' . NV_LANG_DATA . ' SET keywords= :keywords WHERE tid =' . $tid);
                                    $sth->bindParam(':keywords', $keywords_i2, PDO::PARAM_STR);
                                    $sth->execute();
                                }
                            }
                            $db->query('UPDATE ' . TABLE_SHOP_MAIN . '_tags_' . NV_LANG_DATA . ' SET numpro = numpro+1 WHERE tid = ' . $tid);
                        }

                        // insert keyword for table _tags_id
                        try {
                            $sth = $db->prepare('INSERT INTO ' . TABLE_SHOP_MAIN . '_tags_id_' . NV_LANG_DATA . ' (id, tid,  keyword) VALUES (' . $rowcontent['id'] . ', ' . intval($tid) . ', :keyword)');
                            $sth->bindParam(':keyword', $keyword, PDO::PARAM_STR);
                            $sth->execute();
                        } catch (PDOException $e) {
                            $sth = $db->prepare('UPDATE ' . TABLE_SHOP_MAIN . '_tags_id_' . NV_LANG_DATA . ' SET  keyword = :keyword WHERE id = ' . $rowcontent['id'] . ' AND tid=' . intval($tid));
                            $sth->bindParam(':keyword', $keyword, PDO::PARAM_STR);
                            $sth->execute();
                        }
                ...
                foreach ($array_keywords_old as $tid => $keyword) {
                    if (!in_array($keyword, $keywords)) {
                        $db->query('UPDATE ' . TABLE_SHOP_MAIN . '_tags_' . NV_LANG_DATA . ' SET numpro = numpro-1 WHERE tid = ' . $tid);
                        $db->query('DELETE FROM ' . TABLE_SHOP_MAIN . '_tags_id_' . NV_LANG_DATA . ' WHERE id = ' . $rowcontent['id'] . ' AND tid=' . $tid);
                    }
                }
            }
    ...
    $rowcontent = $db->query("SELECT * FROM " . TABLE_SHOP_MAIN . "_rows where id=" . $rowcontent['id'])->fetch();
    ...
    $id_block_content = array( );
    $sql = 'SELECT bid FROM ' . TABLE_SHOP_MAIN . '_block WHERE id=' . $rowcontent['id'];
    $result = $db->query($sql);
...
$array_files = array();
if ($pro_config['download_active']) {
    $sql = 'SELECT id, ' . NV_LANG_DATA . '_title title FROM ' . TABLE_SHOP_MAIN . '_files WHERE status=1';
...
foreach ($global_array_mshops_cat as $mcatid_i => $rowsmcat) {
    $xmtitle_i = '';
    if ($rowsmcat['lev'] > 0) {
        for ($i = 1; $i <= $rowsmcat['lev']; $i++) {
            $xmtitle_i .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
        }
    }
    $rowsmcat['title'] = $xmtitle_i . $rowsmcat['title'];
    $rowsmcat['selected'] = ($mcatid_i == $rowcontent['mcatid']) ? ' selected="selected"' : '';
	$xtpl->assign('ROWSMCAT', $rowsmcat);
    $xtpl->parse('main.rowsmcat');
}
$global_array_shops_cat_site=array();
foreach ($global_array_shops_cat as $catid_i => $rowscat) {
	if($rowscat['idsite']==IDSITE){
		$global_array_shops_cat_site[$catid_i] = $rowscat;
	}
	
}
foreach ($global_array_shops_cat_site as $catid_i => $rowscat) {
    $xtitle_i = '';
    if ($rowscat['lev'] > 0) {
...
if ($typeprice == 1) {
    // List discount
    $sql = 'SELECT * FROM ' . TABLE_SHOP_MAIN . '_discounts ORDER BY add_time DESC';
...
// List discount
$sql = 'SELECT * FROM ' . TABLE_SHOP_MAIN . '_discounts';
$_result = $db->query($sql);
while ($_discount = $_result->fetch()) {
    $_discount['selected'] = ($_discount['did'] == $rowcontent['discount_id']) ? "selected=\"selected\"" : "";
    $xtpl->assign('DISCOUNT', $_discount);
...
// List pro_unit
$sql = 'SELECT id, ' . NV_LANG_DATA . '_title FROM ' . TABLE_SHOP_MAIN . '_units';
...
if ($pro_config['template_active'] and $rowcontent['listcatid'] and !empty($global_array_shops_cat[$rowcontent['listcatid']]['form'])) {
    ...
    if (nv_is_file(NV_BASE_SITEURL . NV_ASSETS_DIR . '/' . $global_config['site_domain'] . '/' . $module_name . '/files_tpl/cat_form_' . $form . '.tpl', NV_ASSETS_DIR . '/' . $global_config['site_domain'] . '/' . $module_name)) {
