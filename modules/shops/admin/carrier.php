...
if ($nv_Request->isset_request('ajax_action', 'post')) {
    ...
    $content = 'NO_' . $id;
    if ($new_vid > 0) {
        $sql = 'SELECT id FROM ' . TABLE_SHOP_MAIN . '_carrier WHERE id!=' . $id . ' ORDER BY weight ASC';
        $result = $db->query($sql);
        $weight = 0;
        while ($row = $result->fetch()) {
            ...
            $sql = 'UPDATE ' . TABLE_SHOP_MAIN . '_carrier SET weight=' . $weight . ' WHERE id=' . $row['id'];
            $db->query($sql);
        }
        $sql = 'UPDATE ' . TABLE_SHOP_MAIN . '_carrier SET weight=' . $new_vid . ' WHERE id=' . $id;
        $db->query($sql);
        $content = 'OK_' . $id;
    }
...
if ($nv_Request->isset_request('delete_id', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
    ...
    if ($id > 0 and $delete_checkss == md5($id . NV_CACHE_PREFIX . $client_info['session_id'])) {
        $weight=0;
        $sql = 'SELECT weight FROM ' . TABLE_SHOP_MAIN . '_carrier WHERE id =' . $db->quote($id);
        $result = $db->query($sql);
        list($weight) = $result->fetch(3);

        $db->query('DELETE FROM ' . TABLE_SHOP_MAIN . '_carrier  WHERE id = ' . $db->quote($id));

        // Xoa bang shops_carrier
        $db->query('DELETE FROM ' . TABLE_SHOP_MAIN . '_shops_carrier WHERE carrier_id = ' . $id);

        if ($weight > 0) {
            $sql = 'SELECT id, weight FROM ' . TABLE_SHOP_MAIN . '_carrier WHERE weight >' . $weight;
            $result = $db->query($sql);
            while (list($id, $weight) = $result->fetch(3)) {
                $weight--;
                $db->query('UPDATE ' . TABLE_SHOP_MAIN . '_carrier SET weight=' . $weight . ' WHERE id=' . intval($id));
            }
        }
...
if ($nv_Request->isset_request('change_active', 'post')) {
    ...
    $sql = 'SELECT id FROM ' . TABLE_SHOP_MAIN . '_carrier WHERE id=' . $id;
    ...
    $sql = 'UPDATE ' . TABLE_SHOP_MAIN . '_carrier SET status=' . $new_status . ' WHERE id=' . $id;
    $db->query($sql);
...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    if (empty($error)) {
        try {
            if (empty($row['id'])) {
                $stmt = $db->prepare('INSERT INTO ' . TABLE_SHOP_MAIN . '_carrier (name, phone, address, logo, description, weight, status) VALUES (:name, :phone, :address, :logo, :description, :weight, 1)');

                $weight = $db->query('SELECT max(weight) FROM ' . TABLE_SHOP_MAIN . '_carrier')->fetchColumn();
                $weight = intval($weight) + 1;
                $stmt->bindParam(':weight', $weight, PDO::PARAM_INT);
            } else {
                $stmt = $db->prepare('UPDATE ' . TABLE_SHOP_MAIN . '_carrier SET name = :name, phone = :phone, address = :address, logo = :logo, description = :description WHERE id=' . $row['id']);
            }
    ...
    $row = $db->query('SELECT * FROM ' . TABLE_SHOP_MAIN . '_carrier WHERE id=' . $row['id'])->fetch();
    if (empty($row)) {
        Header('Location: ' . NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op);
        die();
    }
